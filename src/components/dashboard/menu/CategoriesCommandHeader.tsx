...
import { CheckCircle2, FolderTree, Plus } from 'lucide-react';
...
export function CategoriesCommandHeader({
  totalCount,
  visibleCount,
  onAddCategory,
}: CategoriesCommandHeaderProps) {
  const t = useTranslations('categories');
  const tone = DASHBOARD_NAV_TONES.menuCategories;
    ...
    <header className="border-border bg-card relative overflow-hidden rounded-2xl border shadow-sm">
      ...
      <div className="relative space-y-4 px-4 py-4 ps-6 sm:px-5 sm:ps-7">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Button className="min-h-11 shrink-0 self-start" onClick={onAddCategory}>
          ...
          >
            ...
            >
              <span
                ...
                aria-hidden="true"
              >
                <FolderTree className="size-5 text-white" strokeWidth={1.5} />
              </span>
