#!/usr/bin/env node
/**
 * Sync Doctor Burger "الباستا" category to an exact product list.
 * Usage: node scripts/sync-pasta-menu.mjs [--dry-run]
 */
...
const CATEGORY_AR = 'الباستا';
const CATEGORY_EN = 'Pasta';

const TARGET_PRODUCTS = [
  { name_ar: 'بنا الفريدو', name_en: 'Alfredo', price: 80 },
  { name_ar: 'بولونيز', name_en: 'Bolognese', price: 80 },
  { name_ar: 'مكس تشيز', name_en: 'Mixed Cheese', price: 80 },
  { name_ar: 'نجرسكو', name_en: 'Negrisco', price: 100 },
  { name_ar: 'بشاميل', name_en: 'Bechamel', price: 90 },
  { name_ar: 'ستريبس', name_en: 'Strips', price: 110 },
  { name_ar: 'سوبريم', name_en: 'Supreme', price: 120 },
];
...
function defaultDescriptions(product) {
  return {
    description_ar: `${product.name_ar} — باستا طازجة من دكتور برجر.`,
    description_en: `${product.name_en} — fresh pasta from Doctor Burger.`,
  };
}
...
async function ensureCategory(supabase) {
  ...
  const { data, error: insertErr } = await supabase
    ...
    .insert({
      name_ar: CATEGORY_AR,
      name_en: CATEGORY_EN,
      description_ar: 'قسم الباستا',
      description_en: 'Pasta section',
      is_visible: true,
      sort_order: 0,
    })
    .select('id')
...
async function main() {
  ...
  const outPath = join(ROOT, 'scripts', 'last-pasta-sync-report.json');
  writeFileSync(outPath, JSON.stringify(report, null, 2), 'utf8');
  console.log(`\nReport saved to scripts/last-pasta-sync-report.json`);
