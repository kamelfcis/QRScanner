#!/usr/bin/env node
/**
 * Batch AI product images for Ala Keefak (products missing image_url).
 * Usage: npx tsx scripts/batch-ala-keefak-product-images.ts [--dry-run] [--limit N] [--delay-ms 2000]
 */
...
const REPORT_PATH = join(__dirname, 'last-ala-keefak-ai-images-report.json');
...
async function main() {
  ...
  let query = supabase
    .from('products')
    .select(
      'id, name_ar, name_en, description_ar, description_en, image_url, sort_order, category:categories(name_en, name_ar)'
    )
    .is('image_url', null)
  ...
  console.log(`Ala Keefak batch AI images — ${products.length} product(s) without image_url`);
