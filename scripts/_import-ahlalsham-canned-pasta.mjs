#!/usr/bin/env node
/**
 * Import "المعلبات و المكرونة" category + products from Excel into Ahl El Sham ONLY.
 * Idempotent: safe to re-run (upsert by category name + product name/price key).
 *
 * Usage:
 *   node scripts/_import-ahlalsham-canned-pasta.mjs [--dry-run] [--file path/to/file.xlsx]
 */
...
const DEFAULT_XLSX = 'c:\\Users\\Administrator\\Downloads\\منيو\\منيو المعلبات و المكرونة.xlsx';

const CATEGORY_AR = 'المعلبات و المكرونة';
const CATEGORY_EN = 'Canned Goods & Pasta';
...
async function main() {
  ...
  const reportPath = join(ROOT, 'scripts', 'last-ahlalsham-canned-pasta-import-report.json');
