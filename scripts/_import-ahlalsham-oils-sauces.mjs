#!/usr/bin/env node
/**
 * Import "زيوت الطبخ + الصلصة" category + products from Excel into Ahl El Sham ONLY.
 * Idempotent: safe to re-run (upsert by category name + product name/price key).
 *
 * Usage:
 *   node scripts/_import-ahlalsham-oils-sauces.mjs [--dry-run] [--file path/to/file.xlsx]
 */
...
const DEFAULT_XLSX = 'c:\\Users\\Administrator\\Downloads\\منيو\\زيوت الطبخ + الصلصة.xlsx';

const CATEGORY_AR = 'زيوت الطبخ + الصلصة';
const CATEGORY_EN = 'Cooking Oils & Sauces';
...
async function main() {
  ...
  const reportPath = join(ROOT, 'scripts', 'last-ahlalsham-oils-sauces-import-report.json');
