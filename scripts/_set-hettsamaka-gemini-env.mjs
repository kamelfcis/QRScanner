#!/usr/bin/env node
/**
 * Add GEMINI env vars to Hetta Samaka Vercel project (production/preview/dev).
 */
...
const HETTSAMAKA_PROJECT = 'prj_rYkosytLsBb4MxodD3BAui29uCEN';
...
async function main() {
  ...
  const report = { projectId: HETTSAMAKA_PROJECT, steps: [] };
  report.steps.push({
    key: 'GEMINI_API_KEY',
    action: await upsertEnv(token, HETTSAMAKA_PROJECT, 'GEMINI_API_KEY', geminiKey),
  });
  ...
  for (const key of ['GEMINI_MODEL', 'GEMINI_IMAGE_MODEL']) {
    ...
    report.steps.push({
      key,
      action: await upsertEnv(token, HETTSAMAKA_PROJECT, key, value),
    });
  }
