#!/usr/bin/env node
/**
 * Sync Doctor Burger "سندوتش فينو" category to an exact product list.
 * Usage: node scripts/sync-fino-sandwich-menu.mjs [--dry-run]
 */
...
const CATEGORY_AR = 'سندوتش فينو';
const CATEGORY_EN = 'Fino Sandwich';

const TARGET_PRODUCTS = [
  { name_ar: 'كفتة', name_en: 'Kofta', price: 70 },
  { name_ar: 'شيش طاووق', name_en: 'Shish Tawook', price: 60 },
  { name_ar: 'شاورما', name_en: 'Shawarma', price: 50 },
  { name_ar: 'سجق', name_en: 'Sujuk', price: 50 },
  { name_ar: 'سوسيس', name_en: 'Sausage', price: 50 },
  { name_ar: 'استربس', name_en: 'Strips', price: 45 },
  { name_ar: 'بطاطس', name_en: 'Potatoes', price: 30 },
];
...
/** Match with or without "سندوتش فينو" / "فينو" prefix. */
function productKey(nameAr) {
  let key = normAr(nameAr);
  key = key.replace(/^سندوتش\s+فينو\s+/, '');
  key = key.replace(/^فينو\s+/, '');
  return key;
}

function defaultDescriptions(product) {
  return {
    description_ar: `${product.name_ar} — سندوتش فينو لذيذ من دكتور برجر.`,
    description_en: `${product.name_en} — tasty fino sandwich from Doctor Burger.`,
  };
}
  ...
  if (existing?.length) {
    ...
    if (!DRY_RUN) {
      await supabase
        ...
        .update({
          name_en: CATEGORY_EN,
          description_ar: 'سندوتشات فينو',
          description_en: 'Fino sandwiches',
          is_visible: true,
        })
        .eq('id', keeper.id);
    }
...
async function main() {
  ...
  for (let i = 0; i < TARGET_PRODUCTS.length; i++) {
    ...
    const descMissing = !match.description_ar || !match.description_en;
  ...
  const outPath = join(ROOT, 'scripts', 'last-fino-sandwich-sync-report.json');
  writeFileSync(outPath, JSON.stringify(report, null, 2), 'utf8');
  console.log(`\nReport saved to scripts/last-fino-sandwich-sync-report.json`);
  ...
  console.log('\nVERIFY OK: exactly 7 products with correct prices.');
}
