#!/usr/bin/env node
/**
 * Import "منيو الرز و الدقيق و السكر" category + products from Excel into Ahl El Sham ONLY.
 * Idempotent: safe to re-run (upsert by category name + product name/price key).
 *
 * Usage:
 *   node scripts/_import-ahlalsham-rice-flour-sugar.mjs [--dry-run] [--file path/to/file.xlsx]
 */
...
const DEFAULT_XLSX = 'c:\\Users\\Administrator\\Downloads\\منيو\\منيز الرز و الدقيق و السكر.xlsx';

const CATEGORY_AR = 'منيو الرز و الدقيق و السكر';
const CATEGORY_EN = 'Rice, Flour & Sugar';
...
function parsePrice(raw) {
  if (typeof raw === 'number' && Number.isFinite(raw)) return roundPrice(raw);
...
function productKey(nameAr, price) {
  return `${normAr(nameAr)}|${roundPrice(Number(price))}`;
}
...
async function dedupeCategoryProducts(supabase, categoryId, report) {
  const existing = await fetchCategoryProducts(supabase, categoryId);
  const byKey = new Map();
  for (const p of existing) {
    const key = productKey(p.name_ar, p.dining_price);
  ...
  for (const [, group] of byKey) {
    ...
    for (const extra of extras) {
      report.removed.push({ name_ar: extra.name_ar, price: extra.dining_price, id: extra.id });
      if (!DRY_RUN) {
        const { error } = await supabase.from('products').delete().eq('id', extra.id);
        if (error) report.errors.push(`Delete duplicate "${extra.name_ar}": ${error.message}`);
      }
    }
  }
}
...
async function upsertProducts(supabase, categoryId, products, report) {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    const changed =
      roundPrice(Number(match.dining_price)) !== price ||
      roundPrice(Number(match.takeaway_price)) !== price ||
      match.sort_order !== i ||
...
async function main() {
  ...
  const report = { inserted: [], updated: [], unchanged: [], removed: [], errors: [] };
  await upsertProducts(supabase, category.id, parsed.products, report);
  await dedupeCategoryProducts(supabase, category.id, report);
  ...
  const reportPath = join(ROOT, 'scripts', 'last-ahlalsham-rice-flour-sugar-import-report.json');
