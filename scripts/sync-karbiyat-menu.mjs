#!/usr/bin/env node
/**
 * Sync Doctor Burger "الكربيات" category to an exact product list.
 * Usage: node scripts/sync-karbiyat-menu.mjs [--dry-run]
 */
...
const CATEGORY_AR = 'الكربيات';
const CATEGORY_EN = 'Crepes';

const TARGET_PRODUCTS = [
  { name_ar: 'كريب شاورما فراخ', name_en: 'Crepe Chicken Shawarma', price: 100 },
  { name_ar: 'كريب استريبس', name_en: 'Crepe Strips', price: 110 },
  { name_ar: 'كريب مكس فراخ', name_en: 'Crepe Chicken Mix', price: 125 },
  { name_ar: 'كريب بانيه', name_en: 'Crepe Pane', price: 95 },
  { name_ar: 'كريب بطاطس', name_en: 'Crepe Potatoes', price: 55 },
  { name_ar: 'كريب سوسيس', name_en: 'Crepe Sausage', price: 90 },
  { name_ar: 'كريب سجق', name_en: 'Crepe Sujuk', price: 90 },
  { name_ar: 'كريب مكس لحوم', name_en: 'Crepe Meat Mix', price: 130 },
  { name_ar: 'كريب برجر', name_en: 'Crepe Burger', price: 100 },
  { name_ar: 'كريب كفتة', name_en: 'Crepe Kofta', price: 125 },
  { name_ar: 'كريب زنجر', name_en: 'Crepe Zinger', price: 120 },
  { name_ar: 'كريب تشيكن رانش', name_en: 'Crepe Chicken Ranch', price: 120 },
  { name_ar: 'كريب سوبر كرانشي', name_en: 'Crepe Super Crunchy', price: 120 },
];
...
/** Match with or without "كريب" prefix (and legacy "شاورما" variants). */
function productKey(nameAr) {
  let key = normAr(nameAr);
  key = key.replace(/^كريب\s+/, '');
  key = key.replace(/^شاورما\s+/, '');
  return key;
}

function defaultDescriptions(product) {
  return {
    description_ar: `${product.name_ar} محشو بمكونات طازجة — كريب لذيذ من دكتور برجر.`,
    description_en: `${product.name_en} stuffed with fresh fillings — a tasty crepe from Doctor Burger.`,
  };
}
...
async function ensureCategory(supabase) {
  ...
  const { data, error: insertErr } = await supabase
    ...
    .insert({
      name_ar: CATEGORY_AR,
      name_en: CATEGORY_EN,
      description_ar: 'أطباق الكربيات',
      description_en: 'Carb dishes',
      is_visible: true,
      sort_order: 0,
    })
    .select('id')
...
async function main() {
  ...
  const outPath = join(ROOT, 'scripts', 'last-karbiyat-sync-report.json');
  writeFileSync(outPath, JSON.stringify(report, null, 2), 'utf8');
  console.log(`\nReport saved to scripts/last-karbiyat-sync-report.json`);
  ...
  console.log('\nVERIFY OK: exactly 13 products with correct prices.');
}
