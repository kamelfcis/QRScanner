#!/usr/bin/env node
/**
 * Apply tier 3 migrations on Hetta Samaka ONLY (selnwhfvqhqbzxcuwaho).
 */
...
const MIGRATIONS = ['031_expenses.sql', '032_staff_roles.sql'];
  ...
  const verify = await runSql(
    ...
    `SELECT table_name
      ...
      WHERE table_schema = 'public'
        AND table_name IN ('expenses', 'staff_profiles')
      ORDER BY table_name`,
    'verify'
  );
