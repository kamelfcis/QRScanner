#!/usr/bin/env node
/**
 * Sync Doctor Burger "الوجبات" category to an exact product list.
 * Usage: node scripts/sync-meals-menu.mjs [--dry-run]
 */
...
const CATEGORY_AR = 'الوجبات';
const CATEGORY_EN = 'Meals';

const MEAL_INCLUDES_AR = 'تشتمل على: أرز بسمتي + خيار مخلل + صوص + سلطة';
const MEAL_INCLUDES_EN = 'Includes: basmati rice, pickled cucumber, sauce, and salad';
...
const TARGET_PRODUCTS = [
  ...
  {
    name_ar: 'وجبة شاورما',
    name_en: 'Shawarma Meal',
    dining_price: 110,
    takeaway_price: 110,
  },
  {
    name_ar: 'وجبة شيش طاووق',
    name_en: 'Shish Tawook Meal',
    dining_price: 135,
    takeaway_price: 135,
  },
  {
...
function defaultDescriptions(product) {
  return {
    description_ar: `${product.name_ar}. ${MEAL_INCLUDES_AR}`,
    description_en: `${product.name_en}. ${MEAL_INCLUDES_EN}`,
  };
}

function descriptionsMatch(product, match) {
  const desc = defaultDescriptions(product);
  return (
    match.description_ar?.trim() === desc.description_ar &&
    match.description_en?.trim() === desc.description_en
  );
}
  ...
  if (existing?.length) {
    ...
    if (!DRY_RUN) {
      await supabase
        ...
        .update({
          name_en: CATEGORY_EN,
          description_ar: 'وجبات كاملة',
          description_en: 'Full meals',
          is_visible: true,
        })
        .eq('id', keeper.id);
    }
...
async function main() {
  ...
  for (let i = 0; i < TARGET_PRODUCTS.length; i++) {
    ...
    if (!match) {
      report.upserted.push({ action: 'insert', ...target, ...desc });
      if (!DRY_RUN) {
    ...
    const descChanged = !descriptionsMatch(target, match);

    if (priceChanged || nameChanged || match.sort_order !== i || descChanged) {
  ...
  if (report.upserted.length) {
    ...
    for (const u of report.upserted) {
      console.log(
        `  ${u.action === 'insert' ? '+' : '~'} ${u.name_ar} | ${u.name_en} | ${u.dining_price}`
      );
    }
  }
  ...
  for (const p of report.final) {
    console.log(
      `  ${p.name_ar} | ${p.name_en} | ${p.dining_price} | ${p.description_ar}`
    );
  }

  const outPath = join(ROOT, 'scripts', 'last-meals-sync-report.json');
  writeFileSync(outPath, JSON.stringify(report, null, 2), 'utf8');
  console.log(`\nReport saved to scripts/last-meals-sync-report.json`);
