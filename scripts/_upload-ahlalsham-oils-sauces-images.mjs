#!/usr/bin/env node
/**
 * Upload local product images for Ahl El Sham "زيوت الطبخ + الصلصة" category ONLY.
 * Idempotent: skips products that already have image_url (use --force to re-upload).
 *
 * Usage:
 *   node scripts/_upload-ahlalsham-oils-sauces-images.mjs [--dry-run] [--force] [--dir path]
 */
...
const CATEGORY_ID = '56fd686a-0c18-453a-b370-83c03630493b';
const CATEGORY_AR = 'زيوت الطبخ + الصلصة';
const DEFAULT_IMAGE_DIR =
  'C:\\Users\\Administrator\\Downloads\\زيوت_الطبخ_والصلصة\\زيوت الطبخ + الصلصة';
const REPORT_PATH = join(ROOT, 'scripts', 'last-ahlalsham-oils-sauces-images-report.json');
