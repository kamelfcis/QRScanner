#!/usr/bin/env node
/**
 * Import "قسم التجميل و الخميرة" category + products from Excel into Ahl El Sham ONLY.
 * Idempotent: safe to re-run (upsert by category name + product name/price key).
 *
 * Usage:
 *   node scripts/_import-ahlalsham-beauty-yeast.mjs [--dry-run] [--file path/to/file.xlsx]
 */
...
const DEFAULT_XLSX = 'c:\\Users\\Administrator\\Downloads\\منيو\\قسم التجميل و الخميرة.xlsx';

const CATEGORY_AR = 'قسم التجميل و الخميرة';
const CATEGORY_EN = 'Beauty & Yeast';
...
async function main() {
  ...
  const reportPath = join(ROOT, 'scripts', 'last-ahlalsham-beauty-yeast-import-report.json');
