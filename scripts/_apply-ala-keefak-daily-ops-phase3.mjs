#!/usr/bin/env node
/**
 * Apply migration 039_daily_ops_coupon_recalc.sql on Ala Keefak ONLY (pytmkruoyyhxfktnkpuu).
 */
...
const MIGRATION_FILE = '039_daily_ops_coupon_recalc.sql';
  ...
  const verify = await runSql(
    ...
    `SELECT pg_get_functiondef(p.oid) AS def
     ...
     WHERE n.nspname = 'public'
       AND p.proname = 'recalculate_order_totals'`,
    'verify'
  );
  const def = verify?.[0]?.def ?? '';
  if (!def.includes('evaluate_order_discounts')) {
    throw new Error('recalculate_order_totals missing coupon re-evaluation');
  }
  console.log('Verification: recalculate_order_totals includes evaluate_order_discounts');
}
