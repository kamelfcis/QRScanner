#!/usr/bin/env node
/**
 * Import "منيو الاعشاب" category + products from Excel into Ahl El Sham ONLY.
 * Idempotent: safe to re-run (upsert by category name + product name/price key).
 *
 * Usage:
 *   node scripts/_import-ahlalsham-herbs.mjs [--dry-run] [--file path/to/file.xlsx]
 */
...
const DEFAULT_XLSX = 'c:\\Users\\Administrator\\Downloads\\منيو\\منيو الاعشاب.xlsx';

const CATEGORY_AR = 'منيو الاعشاب';
const CATEGORY_EN = 'Herbs Menu';
...
async function ensureCategory(supabase, categories) {
  const existing = categories.filter(
    (c) => normAr(c.name_ar) === normAr(CATEGORY_AR) || normAr(c.name_ar) === normAr('الاعشاب'),
  );
...
async function main() {
  ...
  const reportPath = join(ROOT, 'scripts', 'last-ahlalsham-herbs-import-report.json');
