#!/usr/bin/env node
/**
 * Apply customer live-status RPC (023) on Ahl El Sham ONLY.
 * Does not change RLS and does not grant public SELECT on orders.
 */
...
const MIGRATION_FILES = ['023_customer_order_status.sql'];
...
SELECT jsonb_build_object(
  ...
  'status_fn', EXISTS (
    ...
    WHERE n.nspname = 'public' AND p.proname = 'get_customer_order_status'
  ),
  'anon_select_orders', EXISTS (
    SELECT 1
    FROM pg_policies
...
const VERIFY_GUARD_SQL = `
SELECT jsonb_build_object(
  'empty_phone_null', public.get_customer_order_status('AS-0001'::text, ''::text) IS NULL,
  'unknown_null', public.get_customer_order_status('NO-SUCH-ORDER'::text, '201000000000'::text) IS NULL
) AS status;
`;

const VERIFY_MATCH_SQL = `
WITH sample AS (
...
SELECT jsonb_build_object(
  'has_sample', EXISTS (SELECT 1 FROM sample),
  'match_ok', CASE
    WHEN NOT EXISTS (SELECT 1 FROM sample) THEN true
    ELSE COALESCE(
      public.get_customer_order_status(
        (SELECT order_number FROM sample),
        (SELECT customer_phone FROM sample)
      )->>'order_number',
      ''
    ) = (SELECT order_number FROM sample)
  END,
  'wrong_phone_null', CASE
    WHEN NOT EXISTS (SELECT 1 FROM sample) THEN true
    ELSE public.get_customer_order_status(
      (SELECT order_number FROM sample),
      '00000000000'
    ) IS NULL
  END,
  'return_keys', CASE
    WHEN NOT EXISTS (SELECT 1 FROM sample) THEN ARRAY[]::text[]
    ELSE (
...
async function main() {
  ...
  const guards = await runSql(secrets, VERIFY_GUARD_SQL, 'verify_guards');
  console.log('Guards:', JSON.stringify(guards, null, 2));
  const match = await runSql(secrets, VERIFY_MATCH_SQL, 'verify_match');
  console.log('Match checks:', JSON.stringify(match, null, 2));

  const status = unwrapStatus(after);
  if (status?.status_fn !== true) {
    throw new Error('get_customer_order_status missing after migration');
  }
  if (status?.anon_select_orders === true) {
    throw new Error('anon SELECT policy on orders is present — aborting');
  }
  const guardStatus = unwrapStatus(guards);
  if (guardStatus?.empty_phone_null !== true || guardStatus?.unknown_null !== true) {
    throw new Error(`unguarded lookup is not null: ${JSON.stringify(guardStatus)}`);
  }

  const matchStatus = unwrapStatus(match);
  if (matchStatus?.match_ok !== true || matchStatus?.wrong_phone_null !== true) {
    throw new Error(`phone gate failed: ${JSON.stringify(matchStatus)}`);
  }
  const keys = matchStatus?.return_keys ?? [];
  const allowed = ['dining_mode', 'order_number', 'status', 'updated_at'];
  if (Array.isArray(keys) && keys.some((key) => !allowed.includes(key))) {
    throw new Error(`RPC returned unexpected keys: ${JSON.stringify(keys)}`);
  }

  console.log('\nAhl El Sham customer order-status RPC complete.');
}
