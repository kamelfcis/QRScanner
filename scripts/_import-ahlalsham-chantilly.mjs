#!/usr/bin/env node
/**
 * Import "كريم شانتيه + الحواج" category + products from Excel into Ahl El Sham ONLY.
 * Idempotent: safe to re-run (upsert by category name + product name/price key).
 *
 * Usage:
 *   node scripts/_import-ahlalsham-chantilly.mjs [--dry-run] [--file path/to/file.xlsx]
 */
...
const DEFAULT_XLSX = 'c:\\Users\\Administrator\\Downloads\\منيو\\كريم شانتيه + الحواج.xlsx';

const CATEGORY_EN = 'Chantilly Cream & Al-Hawaj';

const DRY_RUN = process.argv.includes('--dry-run');
const fileArgIdx = process.argv.indexOf('--file');
const XLSX_PATH = fileArgIdx >= 0 ? process.argv[fileArgIdx + 1] : DEFAULT_XLSX;

function parseCategoryName(filePath) {
  const name = basename(filePath).replace(/\.xlsx$/i, '').trim();
  if (!name) throw new Error(`Could not derive category name from file: ${filePath}`);
  return name;
}
...
function parseExcelProducts(filePath) {
  ...
  const categoryAr = parseCategoryName(filePath);
  ...
  return { sheetName, categoryAr, products, rawCount: raw.length };
}
...
async function fetchCategoryProducts(supabase, categoryId) {
  const { data, error } = await supabase
    .from('products')
    .select('id, name_ar, dining_price, takeaway_price, sort_order, is_popular, is_available')
...
async function ensureCategory(supabase, categories, categoryAr) {
  const existing = categories.filter((c) => normAr(c.name_ar) === normAr(categoryAr));
...
async function main() {
  ...
  console.log(
    `Parsed ${parsed.products.length} unique products (${parsed.rawCount} rows with price) from sheet "${parsed.sheetName}"`,
  );
  console.log(`Category from file: "${parsed.categoryAr}"`);
  ...
  const category = await ensureCategory(supabase, categories, parsed.categoryAr);
  console.log(
    `Category "${parsed.categoryAr}": id=${category.id}, created=${category.created}, sort_order=${category.sort_order}`,
  );
  ...
  const reportPath = join(ROOT, 'scripts', 'last-ahlalsham-chantilly-import-report.json');
