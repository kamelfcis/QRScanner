#!/usr/bin/env node
/**
 * Apply migration 038_daily_ops_order_edit.sql on Ala Keefak ONLY (pytmkruoyyhxfktnkpuu).
 */
...
const MIGRATION_FILE = '038_daily_ops_order_edit.sql';
  ...
  const verify = await runSql(
    ...
    `SELECT proname
     ...
     WHERE n.nspname = 'public'
       AND proname IN ('update_order_item_quantity', 'append_items_to_order', 'recalculate_order_totals')
     ORDER BY proname`,
    'verify'
  );
