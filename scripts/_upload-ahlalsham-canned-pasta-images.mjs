#!/usr/bin/env node
/**
 * Upload local product images for Ahl El Sham "المعلبات و المكرونة" category ONLY.
 * Idempotent: skips products that already have image_url (use --force to re-upload).
 *
 * Usage:
 *   node scripts/_upload-ahlalsham-canned-pasta-images.mjs [--dry-run] [--force] [--dir path]
 */
...
const CATEGORY_ID = '85b5dac0-4522-4e03-936c-8a5c55402cae';
const CATEGORY_AR = 'المعلبات و المكرونة';
const DEFAULT_IMAGE_DIR =
  'C:\\Users\\Administrator\\Downloads\\منيو_المعلبات_و_المكرونة\\منيو المعلبات و المكرونة';
const REPORT_PATH = join(ROOT, 'scripts', 'last-ahlalsham-canned-pasta-images-report.json');
...
async function uploadProductImage(supabase, supabaseUrl, localPath, filename) {
...
async function main() {
  ...
  for (const { image, product, score } of matched) {
    ...
    try {
      ...
      const { path: storagePath, url } = await uploadProductImage(
        supabase,
        ref,
        image.path,
        image.name
