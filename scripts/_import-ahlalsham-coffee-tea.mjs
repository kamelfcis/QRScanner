#!/usr/bin/env node
/**
 * Import "القهوة و الشاي" category + products from Excel into Ahl El Sham ONLY.
 * Idempotent: safe to re-run (upsert by category name + product name/price key).
 *
 * Usage:
 *   node scripts/_import-ahlalsham-coffee-tea.mjs [--dry-run] [--file path/to/file.xlsx]
 */
...
const DEFAULT_XLSX = 'c:\\Users\\Administrator\\Downloads\\منيو\\القهوة و الشاي.xlsx';

const CATEGORY_AR = 'القهوة و الشاي';
const CATEGORY_EN = 'Coffee & Tea';
...
async function main() {
  ...
  const reportPath = join(ROOT, 'scripts', 'last-ahlalsham-coffee-tea-import-report.json');
