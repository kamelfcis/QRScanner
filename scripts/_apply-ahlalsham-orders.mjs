#!/usr/bin/env node
/**
 * Apply orders migrations 019–021 on Ahl El Sham ONLY.
 * Enable settings.features.dashboard_orders (preserve other feature keys).
 * Sync Engaz customers.features.dashboard_orders = true for this customer.
 */
...
VALUES (
  'features',
  '{"ai_product_images": false, "dashboard_orders": true}'::jsonb
)
ON CONFLICT (key) DO UPDATE
SET value = public.settings.value || '{"dashboard_orders": true}'::jsonb,
    updated_at = now();
`;
...
SELECT jsonb_build_object(
  ...
  'order_items_table', EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'order_items'
  ),
  'staff_acknowledged_at', EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'orders' AND column_name = 'staff_acknowledged_at'
  ),
  'delete_fn', EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'delete_orders_in_range'
  ),
  'place_fn', EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
...
async function main() {
  ...
  console.log('  enabling dashboard_orders (preserving existing features)...');
  ...
  if (features && features.dashboard_orders !== true) {
    throw new Error(`dashboard_orders not true after enable: ${JSON.stringify(features)}`);
  }
  console.log('\nAhl El Sham orders migrations + flag complete.');
}
